-- ===============================================
-- CONFIGURAÇÃO SUPABASE - SQL CORRIGIDO
-- Cole TUDO de uma vez no SQL Editor do Supabase
-- ===============================================

-- Limpar tudo primeiro (se necessário)
...
CREATE POLICY "Users can manage own todo tasks" ON todo_tasks FOR ALL 
USING (user_id = auth.uid());
...
BEGIN
    ...
    AND table_name IN (
        ...
        'todo_folders', 'todo_tasks', 'access_tokens', 'access_logs'
    );
    ...
    IF table_count = 12 THEN
        ...
        RAISE NOTICE '✅ Seu Supabase está configurado e pronto para uso!';
        RAISE NOTICE '✅ Agora configure seu sistema através da interface web!';
    ELSE
        RAISE NOTICE '⚠️  AVISO: Apenas % de 12 tabelas foram criadas.', table_count;
    END IF;
END $$; 